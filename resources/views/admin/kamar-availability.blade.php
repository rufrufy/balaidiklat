...
<main class="wrap">
    ...
    @if($tanggalMasuk && $tanggalKeluar)
        <div class="card p-4"><h2 class="h4 mb-3">Ketersediaan {{ \Illuminate\Support\Carbon::parse($tanggalMasuk)->format('d M Y') }} - {{ \Illuminate\Support\Carbon::parse($tanggalKeluar)->format('d M Y') }}</h2><div class="table-responsive"><table class="table align-middle"><thead><tr><th>Jenis Kelas</th><th>Kuota Total</th><th>Sisa Kuota</th><th>Harga/Malam</th><th>Fasilitas</th></tr></thead><tbody>@forelse($kamars as $kamar)<tr><td class="fw-bold">{{ $kamar->jenis_kelas }}</td><td>{{ $kamar->kuota_total }} unit</td><td><span class="badge @if(($kamar->sisa_kuota ?? 0) > 0) bg-success @else bg-danger @endif">{{ $kamar->sisa_kuota ?? 0 }} unit</span></td><td>Rp{{ number_format($kamar->harga_per_malam, 0, ',', '.') }}</td><td class="small">{{ \Illuminate\Support\Str::limit($kamar->fasilitas, 50) ?: '-' }}</td></tr>@empty<tr><td colspan="5" class="text-center text-muted py-4">Tidak ada data jenis kelas.</td></tr>@endforelse</tbody></table></div></div>
    @endif
</main>
</body>
</html>
