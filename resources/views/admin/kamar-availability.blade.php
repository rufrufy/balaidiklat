...
<main class="wrap">
    ...
    @if($tanggalMasuk && $tanggalKeluar)
        <div class="card p-4"><h2 class="h4 mb-3">Kamar tersedia {{ \Illuminate\Support\Carbon::parse($tanggalMasuk)->format('d M Y') }} - {{ \Illuminate\Support\Carbon::parse($tanggalKeluar)->format('d M Y') }}</h2><div class="table-responsive"><table class="table align-middle"><thead><tr><th>Kode</th><th>Nama</th><th>Gedung</th><th>Kapasitas</th><th>Harga/Malam</th><th>Status</th></tr></thead><tbody>@forelse($kamars as $kamar)<tr><td>{{ $kamar->kode }}</td><td>{{ $kamar->nama }}</td><td>{{ $kamar->gedung }}</td><td>{{ $kamar->kapasitas }}</td><td>Rp{{ number_format($kamar->harga_per_malam, 0, ',', '.') }}</td><td>{{ $kamar->status }}</td></tr>@empty<tr><td colspan="6" class="text-center text-muted py-4">Tidak ada kamar available pada tanggal tersebut.</td></tr>@endforelse</tbody></table></div></div>
    @endif
</main>
</body>
</html>
