...
<main class="sheet">
    ...
    <div class="table-responsive"><table class="table table-bordered align-middle"><thead><tr><th>Kode</th><th>Pemesan</th><th>Instansi</th><th>Kegiatan</th><th>Tanggal/Kamar</th><th>Peserta</th><th>Billing</th><th>Status</th></tr></thead><tbody>@forelse($reservasis as $reservasi)<tr><td>{{ $reservasi->kode }}</td><td>{{ $reservasi->nama_pemesan }}<div class="small text-muted">{{ $reservasi->phone_number }}</div></td><td>{{ $reservasi->instansi ?: '-' }}</td><td>{{ $reservasi->kegiatan }}</td><td>@if($reservasi->items->isNotEmpty())@foreach($reservasi->items as $item)@php $ik = $item->jenis_kelas ?? null; @endphp<div>{{ $ik ?: '-' }} ({{ $item->jumlah ?? 1 }} unit)<div class="small text-muted">{{ optional($item->tanggal_masuk)->format('d M Y') }} - {{ optional($item->tanggal_keluar)->format('d M Y') }} | Rp{{ number_format($item->subtotal ?? 0, 0, ',', '.') }}</div></div>@endforeach @else {{ optional($reservasi->tanggal_masuk)->format('d M Y') ?: '-' }} - {{ optional($reservasi->tanggal_keluar)->format('d M Y') ?: '-' }}@php $rk = $reservasi->jenis_kelas ?? null; @endphp<div class="small text-muted">{{ $rk ? $rk.' ('.($reservasi->jumlah ?? 1).' unit)' : '-' }}</div>@endif</td><td>{{ $reservasi->jumlah_peserta }}</td><td>Rp{{ number_format($reservasi->total_harga, 0, ',', '.') }}<div class="small text-muted">{{ $reservasi->payment_status }}</div></td><td>{{ $reservasi->status }}</td></tr>@empty<tr><td colspan="8" class="text-center text-muted py-4">Tidak ada reservasi pada bulan ini.</td></tr>@endforelse</tbody></table></div>
</main>
</body>
</html>
